...
console.log(project);
  ...
  const view_units_byid = async (pageNo = 1) => {
    try {
      ...
      const resp = await api.get(
        `viewprojectunits/${id}?page=${pageNo}&limit=${limit}`
      );
  ...
  const deleteall1 = (index) => {
    ...
    const newregistrationno = project.registration_no.filter(
      (_, i) => i !== index
    );
 ...
 const handlepicchange = async (index, event) => {
  ...
  const deleteall4 = (index) => {
    ...
    const newcalculationtype = payments.calculation_type.filter(
      (_, i) => i !== index
    );
  ...
  useEffect(() => {
    const initialCheckedItems = checkboxItems.map((item) =>
      project.basic_aminities.includes(item)
    );
  ...
  useEffect(() => {
    const initialCheckedItems = checkboxItems1.map((item) =>
      project.features_aminities.includes(item)
    );
  ...
  const handleCheckboxChange1 = (index) => {
    ...
    setproject((prevState) => {
      const updatedBasicAminities = updatedCheckedItems1[index]
        ...
        : prevState.features_aminities.filter(
            (item) => item !== selectedAminity1
          );
  ...
  const deleteall12 = (index) => {
    ...
    const newlinkedcontact = units.linkded_contact.filter(
      (_, i) => i !== index
    );
  ...
  const uniqueUnits = project?.add_unit?.filter(
    (value, index, self) =>
      index === self.findIndex((t) => t.unit_no === value.unit_no)
  );
  ...
  const adddestination = () => {
    if (
      ...
      destinations.name_of_destination &&
      destinations.distance &&
      destinations.measurment
    ) {
  ...
  const deletedestination = (index) => {
    // Filter out the destination at the given index
    const newDestinationDetails = project.nearby_aminities.filter(
      (_, i) => i !== index
    );
    ...
    setproject((prevProject) => {
      ...
      if (type === "Agricultural") {
        if (category.includes("Agricultural")) {
          ...
          return {
            ...prevProject,
            category: category.filter((item) => item !== "Agricultural"),
          };
        } else {
        ...
        if (category.includes("Agricultural")) {
          ...
          toast.error(
            "if you select Agricultural then you can't select more category"
          );
          ...
          if (category.includes(type)) {
            ...
            return {
              ...prevProject,
              category: category.filter((item) => item !== type),
            };
          } else {
  ...
  const handleTypeClick3 = (type) => {
    setblock((prevblock) => {
      ...
      if (prevblock.category.includes(type)) {
        ...
        return {
          ...prevblock,
          category: prevblock.category.filter((item) => item !== type),
        };
      } else {
  ...
  const teamlist = ["Sales", "Marketing", "Post Sales", "Pre Sales"];
      ...
      project.add_size.forEach((item) => {
        ...
        if (
          ...
          units.share.forEach((fraction) => {
            ...
            console.log(
              `Effective Area with share ${fraction}:`,
              effectiveArea
            );
  ...
  useEffect(() => {
    ...
    if (Array.isArray(units.linkded_contact)) {
      ...
      const validContacts = units.linkded_contact.filter(
        ...
        const results = allSuggestions.filter((contact) =>
          validContacts.some(
            (linkedContact) =>
              ...
              linkedContact
                .trim()
                .toLowerCase() // Matching against lowercase version of the input text
                .split(" ")
                .some((word) =>
                  contact.first_name?.toLowerCase().includes(word)
                ) // Matching against each word
          )
        );
      ...
      console.warn(
        "Expected an array for units.linkded_contact, but received:",
        units.linkded_contact
      );
    ...
    try {
      ...
      for (let i = 0; i < total; i += batchSize) {
        ...
        try {
          const resp = await api.put(
            "updateprojectforinventoriesbulk",
            batch,
            config
          );
          console.log(resp);
          ...
          if (resp.status === 200) {
            successCount += batch.length;
            toast.success(`Updated ${successCount}/${total} units`, {
              autoClose: 2000,
            });
          } else {
            failCount += batch.length;
      ...
      if (successCount === total) {
        ...
        Swal.fire({
          icon: "warning",
          title: "Partial Updated Complete",
          html: `<b>${successCount}</b> units updated successfully.<br><b>${failCount}</b> failed.`,
        });
      }
    } catch (error) {
      console.error(error);
  ...
  const handleFileChangesize1 = (event) => {
    ...
    setIsLoading(true); // Start loading
    setSelectedFilesize(file); // Store file for later use
  ...
  const checkForDuplicatessize = async (contacts) => {
    try {
      ...
      // Fetch existing units
      const response = await api.get("viewproject");
      const allsize = response.data.project.flatMap(
        (project) => project.add_size
      );
      ...
      contacts.forEach((contact) => {
        ...
        const isDuplicate = allsize.some(
          (unit) =>
            unit.category.trim().toLowerCase() ===
              contact.category.trim().toLowerCase() &&
            unit.size_name.trim().toLowerCase() ===
              contact.size_name.trim().toLowerCase()
        );
  ...
  const checkForDuplicatesblock = async (contacts) => {
    try {
      ...
      // Fetch existing units
      const response = await api.get("viewproject");
      const allblock = response.data.project.flatMap(
        (project) => project.add_block
      );
      ...
      contacts.forEach((contact) => {
        // Check if unit is duplicate (case-insensitive and trimmed)
        const isDuplicate = allblock.some(
          (unit) =>
            String(unit.block_name).trim().toLowerCase() ===
              String(contact.block_name).trim().toLowerCase() &&
            Array.isArray(unit.category) &&
            Array.isArray(contact.category) &&
            unit.category.map((item) => item.trim().toLowerCase()).join(",") ===
              contact.category
  ...
  const unitdata = [
    {
      ...
      owner_mobile_no: "9944554411",
      owner_mobile_type: "personal",
      owner_email: "alex@gmail.com",
      owner_email_type: "personal",
      owner_father_name: "jon",
      owner_hno: "f13",
      owner_area: "bishanpura",
      owner_location: "sec 58",
      owner_city: "noida",
      owner_pincode: "201301",
      owner_state: "up",
      owner_country: "india",
      associated_hno: "f13",
      associated_area: "bishanpura",
      associated_location: "sec58",
      associated_city: "noida",
      associated_pincode: "201301",
      associated_state: "up",
      associated_country: "india",
      associated_title: "Mr.",
      associated_first_name: "jon",
      associated_last_name: "dow",
      associated_country_code: "91",
      associated_mobile_no: "9454226644",
      associated_mobile_type: "personal",
      associated_email: "jon@gmail.com",
      associated_email_type: "home",
            ...
            <div className="col-md-12 border-right">
              <div className="p-3 py-5">
                ...
                >
                  ...
                  <div className="col-md-6 custom-input">
                    <label className="form-label">Secondary Developer</label>
                    <select
                      ...
                      onChange={(e) =>
                        setproject({
                          ...project,
                          secondary_developer: e.target.value,
                        })
                      }
                    >
                  ...
                  <div className="col-md-5 custom-input">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setproject({ ...project, rera_number: e.target.value })
                      }
                    />
                  </div>
                  ...
                  >
                    ...
                    {All_Category.map((type) => (
                      <div
                        className="col-md-2  custom-input"
                        key={type}
                      ...
                      >
                        <button
                          ...
                          style={{
                            backgroundColor: isSelected(type.lookup_value)
                              ? "#28a745"
                              : "#f8f9fa", // green or light gray
                            color: isSelected(type.lookup_value)
                              ? "white"
                              : "#333",
                 ...
                 <div className="col-md-6  custom-input">
                  ...
                  <div className="col-md-2 custom-input">
                    ...
                    <input
                      ...
                      required="true"
                      onChange={(e) =>
                        setproject({ ...project, land_area: e.target.value })
                      }
                    />
                  </div>
                  <div className="col-md-2 custom-input">
                    ...
                    <select
                      ...
                      required="true"
                      onChange={(e) =>
                        setproject({ ...project, measurment1: e.target.value })
                      }
                    >
                  ...
                  <div className="col-md-2 custom-input">
                    ...
                    <input
                      type="number"
                      value={project.total_block}
                      className="form-control form-control-sm"
                      required="true"
                      onChange={(e) =>
                        setproject({ ...project, total_block: e.target.value })
                      }
                    />
                  </div>
                  <div className="col-md- custom-input" id="totalfloors">
                    <label className="form-label">TOTAL Floor</label>
                    <input
                      type="number"
                      value={project.total_floor}
                      className="form-control form-control-sm"
                      required="true"
                      onChange={(e) =>
                        setproject({ ...project, total_floor: e.target.value })
                      }
                    />
                  </div>
                  <div className="col-md-2 custom-input">
                    ...
                    <input
                      ...
                      required="true"
                      onChange={(e) =>
                        setproject({ ...project, total_units: e.target.value })
                      }
                    />
                  </div>
                  <div className="col-md-2 custom-input"></div>
                 ...
                 <div className="col-md-6 custom-input">
                      ...
                      {ownersList.map((name) => (
                        <MenuItem key={name} value={name}>
                          <Checkbox
                            checked={
                              project.owner
                                ? project.owner.indexOf(name) > -1
                                : owners.indexOf(name) > -1
                            }
                          />
                          <ListItemText primary={name} />
                        </MenuItem>
                      ...
                      {teamlist.map((name) => (
                        <MenuItem key={name} value={name}>
                          <Checkbox
                            checked={
                              project.team
                                ? project.team.indexOf(name) > -1
                                : teams.indexOf(name) > -1
                            }
                          />
                          <ListItemText primary={name} />
                        </MenuItem>
                  ...
                  <div className="col-md-6 custom-input">
                    ...
                    <select
                      className="form-control form-control-sm"
                      onChange={(e) =>
                        setproject({ ...project, visible_to: e.target.value })
                      }
                    >
                  ...
                  >
                    ...
                    <div className="col-md-8">
                      ...
                      <input
                        ...
                        className="form-control form-control-sm"
                        onChange={(e) =>
                          setproject({ ...project, street: e.target.value })
                        }
                      />
                    </div>
                    <div className="col-md-4"></div>
                    <div className="col-md-4">
                      ...
                      <input
                        ...
                        className="form-control form-control-sm"
                        onChange={(e) =>
                          setproject({ ...project, locality: e.target.value })
                        }
                      />
                    </div>
                    <div className="col-md-4">
                    ...
                    <div className="col-md-4">
                      ...
                      <input
                        ...
                        className="form-control form-control-sm"
                        onChange={(e) =>
                          setproject({ ...project, zip: e.target.value })
                        }
                      />
                    </div>
                    <div className="col-md-6">
                    ...
                    <div className="col-md-6">
                      ...
                      <select
                        className="form-control form-control-sm"
                        onChange={(e) =>
                          setproject({ ...project, country: e.target.value })
                        }
                      >
                            ...
                            >
                              ...
                              {project.sub_category.map((item) => (
                                <MenuItem key={item} value={item}>
                                  <Checkbox
                                    checked={
                                      block.sub_category.indexOf(item) > -1
                                    }
                                  />
                              ...
                              <div className="col-md-10">
                                ...
                                >
                                  <MenuItem value="select-all">
                                    <Checkbox
                                      checked={
                                        parkings.length === parking.length
                                      }
                                    />
                                    <ListItemText
                                      primary={"---select all---"} //
                                  ...
                                  {parking.map((name) => (
                                    <MenuItem key={name} value={name}>
                                      <Checkbox
                                        checked={parkings.indexOf(name) > -1}
                                      />
                                      <ListItemText primary={name} />
                                    </MenuItem>
                                  ))}
                          ...
                          {pendingContacts.map((entry, index) => (
                            <div key={index} className="col-md-4 mb-3">
                              <div className="p-2 border rounded bg-light">
                                ...
                                <p className="mb-0">
                                  <strong>Unit No:</strong> {entry.unit_no}
                                </p>
                              </div>
                            </div>
                          ))}
            ...
            <Modal show={show8} onHide={handleClose8} size="lg">
              ...
              <Modal.Body>
                <input
                  type="file"
                  accept=".xlsx,.xls"
                  onChange={handleFileChangeblock}
                  id="import-file"
                />
              </Modal.Body>
              <Modal.Footer>
