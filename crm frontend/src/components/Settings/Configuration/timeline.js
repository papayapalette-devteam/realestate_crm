...
function Timeline() {
  ...
  const onDelete = async (row) => {
    try {
      const confirmResult = await Swal.fire({
        title: "Are you sure?",
        text: "Do you really want to delete this Timeline?",
        icon: "warning",
  ...
  const add_timeline = async () => {
    try {
      setloading(true);
      const resp = await api.post("api/SaveLookup", {
        lookup_id: lookup_id ? lookup_id : null,
        lookup_type: "timeline",
        lookup_value: Timeline.timeline,
      });
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className=" p-6 mb-2">
            <h3 className="text-2xl font-bold text-gray-800">
              Enter Details for Timeline
            </h3>
            <p className="text-gray-500 mt-1">
              Add or update the required details for timeline to keep records
              accurate.
            </p>
          </div>
