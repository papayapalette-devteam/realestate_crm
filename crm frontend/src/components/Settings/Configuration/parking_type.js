...
function ParkingType() {
  ...
  const onDelete = async (row) => {
    try {
      const confirmResult = await Swal.fire({
        title: "Are you sure?",
        text: "Do you really want to delete this Parking Type?",
        icon: "warning",
      ...
      if (resp.status === 200) {
        setTimeout(() => {
          Swal.fire({
            icon: "success",
            title: "Parking Type Deleted",
            text: "Parking Type Deleted Successfully...",
  ...
  const allcolumns = [
    { id: "sno", name: "#" },
    { id: "lookup_value", name: "Parking Type" },
  ...
  const add_parking_type = async () => {
    try {
      setloading(true);
      const resp = await api.post("api/SaveLookup", {
        lookup_id: lookup_id ? lookup_id : null,
        lookup_type: "parking_type",
        lookup_value: Parking_Type.parking_type,
      });
      ...
      if (resp.status === 200) {
        setTimeout(() => {
          Swal.fire({
            icon: "success",
            title: "Parking Type Added",
            text: "Parking Type Added Successfully...",
            showConfirmButton: true,
            customClass: {
              popup: "small-swal-popup",
              confirmButton: "my-swal-button",
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className=" p-6 mb-2">
            <h3 className="text-2xl font-bold text-gray-800">
              Enter Details for Parking Type
            </h3>
            <p className="text-gray-500 mt-1">
              Add or update the required details for parking type to keep records
              accurate.
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  type="text"
                  name="parking_type"
                  defaultValue={Parking_Type.parking_type}
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            <div className="overflow-auto max-h-[700px]">
              <table className="min-w-full text-left border-collapse">
                ...
                <tbody>
                  {All_Parking_Type.map((item, index) => (
                    <tr
                      key={index}
