...
function ProfessionCategory() {
  const [loading, setloading] = useState(false);
  const [Profession_Category, setProfession_Category] = useState({
    profession_category: "",
  });
  ...
  const onEdit = (row) => {
    ...
    setProfession_Category({
      profession_category: row.lookup_value,
    });
  };
  ...
  const onDelete = async (row) => {
    try {
      const confirmResult = await Swal.fire({
        title: "Are you sure?",
        text: "Do you really want to delete this Profession Category?",
        icon: "warning",
      ...
      if (resp.status === 200) {
        setTimeout(() => {
          Swal.fire({
            icon: "success",
            title: "Profession Category Deleted",
            text: "Profession Category Deleted Successfully...",
  ...
  const allcolumns = [
    { id: "sno", name: "#" },
    { id: "lookup_value", name: "Profession Category" },
  ...
  const add_profession_category = async () => {
    try {
      setloading(true);
      const resp = await api.post("api/SaveLookup", {
        lookup_id: lookup_id ? lookup_id : null,
        lookup_type: "profession_category",
        lookup_value: Profession_Category.profession_category,
      });
      ...
      if (resp.status === 200) {
        setTimeout(() => {
          Swal.fire({
            icon: "success",
            title: "Profession Category Added",
            text: "Profession Category Added Successfully...",
            showConfirmButton: true,
            customClass: {
              popup: "small-swal-popup",
              confirmButton: "my-swal-button",
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className=" p-6 mb-2">
            <h3 className="text-2xl font-bold text-gray-800">
              Enter Details for Profession Category
            </h3>
            <p className="text-gray-500 mt-1">
              Add or update the required details of profession category to keep records
              accurate.
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow p-6 mb-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  defaultValue={Profession_Category.profession_category}
            ...
            <button
              ...
              onClick={add_profession_category}
            >
              Submit
            </button>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            <div className="overflow-auto max-h-[700px]">
              <table className="min-w-full text-left border-collapse">
                ...
                <tbody>
                  {All_Profession_Category?.map((item, index) => (
                    <tr
                      key={index}
