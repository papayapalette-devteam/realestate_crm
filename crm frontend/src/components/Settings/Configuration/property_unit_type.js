...
function PropertyUnitType() {
  const [loading, setloading] = useState(false);
  const [Property_Unit_Type, setProperty_Unit_Type] = useState({
    property_unit_type: "",
    property_sub_type: "",
  });
  ...
  const onEdit = (row) => {
    ...
    setProperty_Unit_Type({
      property_unit_type: row.lookup_value,
      property_sub_type: row.parent_lookup_value,
    });
  };
  ...
  const onDelete = async (row) => {
    try {
      const confirmResult = await Swal.fire({
        title: "Are you sure?",
        text: "Do you really want to delete this Property Unit Type?",
        icon: "warning",
      ...
      if (resp.status === 200) {
        setTimeout(() => {
          Swal.fire({
            icon: "success",
            title: "Property Unit Type Deleted",
            text: "Property Unit Type Deleted Successfully...",
  ...
  const allcolumns = [
    { id: "sno", name: "#" },
    { id: "parent_lookup_value", name: "Property Sub Type" },
    { id: "lookup_value", name: "Property Unit Type" },
    { id: "action", name: "Action" },
  ];

  //================================ get property sub type start==========================================
  ...
  const getall_property_sub_type = async () => {
    try {
      ...
      setAll_Property_Sub_Type(resp.data.data);
      setRowCount(resp.data.total);
    } catch (error) {
      console.log(error);
    } finally {
  ...
  //================================== get property sub type end=====================================

  const handlechange = (e) => {
    const { name, value, checked, type } = e.target;

    setProperty_Unit_Type((prev) => {
  ...
  const add_property_unit_type = async () => {
    try {
      setloading(true);
      const resp = await api.post("api/SaveLookup", {
        lookup_id: lookup_id ? lookup_id : null,
        lookup_type: "property_unit_type",
        lookup_value: Property_Unit_Type.property_unit_type,
        parent_lookup_value: Property_Unit_Type.property_sub_type,
      });
      ...
      if (resp.status === 200) {
        setTimeout(() => {
          Swal.fire({
            icon: "success",
            title: "Property Unit Type Added",
            text: "Property Unit Type Added Successfully...",
            showConfirmButton: true,
            customClass: {
              popup: "small-swal-popup",
              confirmButton: "my-swal-button",
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className=" p-6 mb-2">
            <h3 className="text-2xl font-bold text-gray-800">
              Enter Details for Property Unit Type
            </h3>
            <p className="text-gray-500 mt-1">
              Add or update the required details of property unit type to keep records
              accurate.
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow p-6 mb-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  defaultValue={Property_Unit_Type.property_unit_type}
                  onChange={handlechange}
                  placeholder="Property Unit Type"
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    defaultValue={Property_Unit_Type.property_sub_type}
                    onChange={handlechange}
                    onClick={() => {
                      if (All_Property_Sub_Type.length === 0) {
                  ...
                  >
                    ...
                    {loadingstate ? (
                      ...
                      <>
                        <option>---select property sub type---</option>

                        {All_Property_Sub_Type.map((item) => (
            ...
            <button
              ...
              onClick={add_property_unit_type}
            >
              Submit
            </button>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            <div className="overflow-auto max-h-[700px]">
              <table className="min-w-full text-left border-collapse">
                ...
                <tbody>
                  {All_Property_Unit_Type?.map((item, index) => (
                    <tr
                      key={index}
