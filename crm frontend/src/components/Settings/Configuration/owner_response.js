...
function OwnerResponse() {
  ...
  const [Owner_Response,setOwner_Response] = useState({
    owner_response: "",
  });
  ...
  const onEdit = (row) => {
    ...
    setOwner_Response({
      owner_response: row.lookup_value,
    });
  };
  ...
  const onDelete = async (row) => {
    try {
      const confirmResult = await Swal.fire({
        title: "Are you sure?",
        text: "Do you really want to delete this Owner Response?",
        icon: "warning",
      ...
      if (resp.status === 200) {
        setTimeout(() => {
          Swal.fire({
            icon: "success",
            title: "Owner Response Deleted",
            text: "Owner Response Deleted Successfully...",
  ...
  const allcolumns = [
    { id: "sno", name: "#" },
    { id: "lookup_value", name: "Owner Response" },
  ...
  const add_owner_response = async () => {
    try {
      setloading(true);
      const resp = await api.post("api/SaveLookup", {
        lookup_id: lookup_id ? lookup_id : null,
        lookup_type: "owner_response",
        lookup_value: Owner_Response.owner_response,
      });
      ...
      if (resp.status === 200) {
        setTimeout(() => {
          Swal.fire({
            icon: "success",
            title: "Owner Response Added",
            text: "Owner Response Added Successfully...",
            showConfirmButton: true,
            customClass: {
              popup: "small-swal-popup",
              confirmButton: "my-swal-button",
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className=" p-6 mb-2">
            <h3 className="text-2xl font-bold text-gray-800">
              Enter Details for Owner Response
            </h3>
            <p className="text-gray-500 mt-1">
              Add or update the required details for owner response to keep records
              accurate.
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Owner Response For Feedback Form
                </label>
                <input
                  type="text"
                  name="owner_response"
                  defaultValue={Owner_Response.owner_response}
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            <div className="overflow-auto max-h-[700px]">
              <table className="min-w-full text-left border-collapse">
                ...
                <tbody>
                  {All_Owner_Response?.map((item, index) => (
                    <tr
                      key={index}
