...
function BuiltupType() {
  const [loading, setloading] = useState(false);
  const [Builtup_Type, setBuiltup_Type] = useState({
    builtup_type: "",
    sub_category:""
  });
  ...
  const onEdit = (row) => {
    ...
    setBuiltup_Type({
      builtup_type: row.lookup_value,
      sub_category: row.parent_lookup_value,
    });
  };
  ...
  const onDelete = async (row) => {
    try {
      const confirmResult = await Swal.fire({
        title: "Are you sure?",
        text: "Do you really want to delete this Builtup Type?",
        icon: "warning",
      ...
      if (resp.status === 200) {
        setTimeout(() => {
          Swal.fire({
            icon: "success",
            title: "Builtup Type Deleted",
            text: "Builtup Type Deleted Successfully...",
  ...
  const allcolumns = [
    { id: "sno", name: "#" },
    { id: "lookup_value", name: "Builtup Type" },
    { id: "parent_lookup_value", name: "Sub Category" },
...
//=============================   get subcategory end====================================
  ...
  const add_builtup_type = async () => {
    try {
      setloading(true);
      const resp = await api.post("api/SaveLookup", {
        lookup_id: lookup_id ? lookup_id : null,
        lookup_type: "builtup_type",
        lookup_value: Builtup_Type.builtup_type,
        parent_lookup_value: Builtup_Type.sub_category,
      });
      ...
      if (resp.status === 200) {
        setTimeout(() => {
          Swal.fire({
            icon: "success",
            title: "Builtup Type Added",
            text: "Builtup Type Added Successfully...",
            showConfirmButton: true,
            customClass: {
              popup: "small-swal-popup",
              confirmButton: "my-swal-button",
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className=" p-6 mb-2">
            <h3 className="text-2xl font-bold text-gray-800">
              Enter Details for Builtup Type
            </h3>
            <p className="text-gray-500 mt-1">
              Add or update the required details for builtup type to keep records
              accurate.
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  type="text"
                  name="builtup_type"
                  defaultValue={Builtup_Type.builtup_type}
                  onChange={handlechange}
                  placeholder="Builtup Type"
               ...
               <div>
                ...
                <div className="relative">
                  <select
                    type="text"
                    name="sub_category"
                    defaultValue={Builtup_Type.sub_category}
                    onChange={handlechange}
                    onClick={() => {
                  ...
                  >
                    ...
                    {loadingSubCategory ? (
                      ...
                      <>
                        <option>
                          {Builtup_Type?.sub_category ||
                            "---select sub category---"}
                        </option>
                        <option
                          style={{
                            display: Builtup_Type.sub_category
                              ? "block"
                              : "none",
                          }}
                        >
                        ...
                        {All_Profession_Sub_Category.map((item) => (
                          <option key={item.lookup_value} value={item.lookup_value}>
                            {item.lookup_value}
                          </option>
                        ))}
                      </>
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            <div className="overflow-auto max-h-[700px]">
              <table className="min-w-full text-left border-collapse">
                ...
                <tbody>
                  {All_Builtup_Type.map((item, index) => (
                    <tr
                      key={index}
