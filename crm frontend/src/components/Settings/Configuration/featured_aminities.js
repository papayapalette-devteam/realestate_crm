...
function FeaturedAminities() {
  const [loading, setloading] = useState(false);
  const [Featured_Aminities, setFeatured_Aminities] = useState({
    featured_aminities: "",
  });
  ...
  const [All_Featured_Aminities, setAll_Featured_Aminities] = useState([]);
  const getall_featured_aminities = async (
    ...
    try {
      ...
      setAll_Featured_Aminities(resp.data.data);
  ...
  const onEdit = (row) => {
    ...
    setFeatured_Aminities({
      featured_aminities: row.lookup_value,
    });
  };
  ...
  const onDelete = async (row) => {
    try {
      const confirmResult = await Swal.fire({
        title: "Are you sure?",
        text: "Do you really want to delete this Featured Aminities?",
        icon: "warning",
      ...
      if (resp.status === 200) {
        setTimeout(() => {
          Swal.fire({
            icon: "success",
            title: "Featured Aminities Deleted",
            text: "Featured Aminities Deleted Successfully...",
  ...
  const allcolumns = [
    { id: "sno", name: "#" },
    { id: "lookup_value", name: "Featured Aminities" },
  ...
  const add_featured_aminities = async () => {
    try {
      setloading(true);
      const resp = await api.post("api/SaveLookup", {
        lookup_id: lookup_id ? lookup_id : null,
        lookup_type: "featured_aminities",
        lookup_value: Featured_Aminities.featured_aminities,
      });
      ...
      if (resp.status === 200) {
        setTimeout(() => {
          Swal.fire({
            icon: "success",
            title: "Basic Aminities Added",
            text: "Basic Aminities Added Successfully...",
            showConfirmButton: true,
            customClass: {
              popup: "small-swal-popup",
              confirmButton: "my-swal-button",
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className=" p-6 mb-2">
            <h3 className="text-2xl font-bold text-gray-800">
              Enter Details for Featured Aminities
            </h3>
            <p className="text-gray-500 mt-1">
              Add or update the required details for featured aminities to keep records
              accurate.
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow p-6 mb-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  type="text"
                  name="featured_aminities"
                  defaultValue={Featured_Aminities.featured_aminities}
                  onChange={handlechange}
                  placeholder="Featured Aminities"
            ...
            <button
              ...
              onClick={add_featured_aminities}
            >
              Submit
            </button>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            <div className="overflow-auto max-h-[700px]">
              <table className="min-w-full text-left border-collapse">
                ...
                <tbody>
                  {All_Featured_Aminities.map((item, index) => (
                    <tr
                      key={index}
...
export default FeaturedAminities;
