...
function BasicAminities() {
  const [loading, setloading] = useState(false);
  const [Basic_Aminities, setBasic_Aminities] = useState({
    basic_aminities: "",
  });
  ...
  const [All_Basic_Aminities, setAll_Basic_Aminities] = useState([]);
  const getall_basic_aminities = async (
    ...
    try {
      ...
      setAll_Basic_Aminities(resp.data.data);
  ...
  const onEdit = (row) => {
    ...
    setBasic_Aminities({
      basic_aminities: row.lookup_value,
    });
  };
  ...
  const onDelete = async (row) => {
    try {
      const confirmResult = await Swal.fire({
        title: "Are you sure?",
        text: "Do you really want to delete this Basic Aminities?",
        icon: "warning",
      ...
      if (resp.status === 200) {
        setTimeout(() => {
          Swal.fire({
            icon: "success",
            title: "Basic Aminities Deleted",
            text: "Basic Aminities Deleted Successfully...",
  ...
  const allcolumns = [
    { id: "sno", name: "#" },
    { id: "lookup_value", name: "Basic Aminities" },
  ...
  const add_basic_aminities = async () => {
    try {
      setloading(true);
      const resp = await api.post("api/SaveLookup", {
        lookup_id: lookup_id ? lookup_id : null,
        lookup_type: "basic_aminities",
        lookup_value: Basic_Aminities.basic_aminities,
      });
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className=" p-6 mb-2">
            <h3 className="text-2xl font-bold text-gray-800">
              Enter Details for Basic Aminities
            </h3>
            <p className="text-gray-500 mt-1">
              Add or update the required details for basic aminities to keep records
              accurate.
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow p-6 mb-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  type="text"
                  name="basic_aminities"
                  defaultValue={Basic_Aminities.basic_aminities}
                  onChange={handlechange}
                  placeholder="Basic Aminities"
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            <div className="overflow-auto max-h-[700px]">
              <table className="min-w-full text-left border-collapse">
                ...
                <tbody>
                  {All_Basic_Aminities.map((item, index) => (
                    <tr
                      key={index}
