...
function ProjectStatus() {
  ...
  const onEdit = (row) => {
    ...
    setProject_Status({
      project_status: row.lookup_value,
    });
  };
  ...
  const onDelete = async (row) => {
    try {
      const confirmResult = await Swal.fire({
        title: "Are you sure?",
        text: "Do you really want to delete this Project Status?",
        icon: "warning",
      ...
      if (resp.status === 200) {
        setTimeout(() => {
          Swal.fire({
            icon: "success",
            title: "Project Status Deleted",
            text: "Project Status Deleted Successfully...",
  ...
  const allcolumns = [
    { id: "sno", name: "#" },
    { id: "lookup_value", name: "Project Status" },
  ...
  const add_project_status = async () => {
    try {
      setloading(true);
      const resp = await api.post("api/SaveLookup", {
        lookup_id: lookup_id ? lookup_id : null,
        lookup_type: "project_status",
        lookup_value: Project_Status.project_status,
      });
      ...
      if (resp.status === 200) {
        setTimeout(() => {
          Swal.fire({
            icon: "success",
            title: "Project Status Added",
            text: "Project Status Added Successfully...",
            showConfirmButton: true,
            customClass: {
              popup: "small-swal-popup",
              confirmButton: "my-swal-button",
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className=" p-6 mb-2">
            <h3 className="text-2xl font-bold text-gray-800">
              Enter Details for Project Status
            </h3>
            <p className="text-gray-500 mt-1">
              Add or update the required details for project status to keep records
              accurate.
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  type="text"
                  name="project_status"
                  defaultValue={Project_Status.project_status}
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            <div className="overflow-auto max-h-[700px]">
              <table className="min-w-full text-left border-collapse">
                ...
                <tbody>
                  {All_Project_Status.map((item, index) => (
                    <tr
                      key={index}
