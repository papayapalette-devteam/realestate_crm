...
const {
  ...
  getGroupedData,
} = require("../controllers/contact_details");
const {
  lead_info,
  leadinfo_find,
...
const {
  ...
  update_developer,
  remove_developer,
} = require("../controllers/add_developer");
...
const {
  ...
  view_mailtask_Byid,
  update_mailtask,
} = require("../controllers/mail_task_form");
const {
  call_task_form,
  view_call,
  update_calltask,
  remove_calltask,
  view_calltask_Byid,
} = require("../controllers/call_task_form");
const {
  meeting_task_form,
  viewmeeting_task,
  remove_meetingtask,
  view_meetingtask_Byid,
  update_meetingtask,
} = require("../controllers/meeting_task_form");
const {
  site_visit_form,
  view_site,
  remove_sitevisittask,
  view_sitevisittask_Byid,
  update_sitevisittask,
} = require("../controllers/site_visit_form");
const {
  booking_details,
  view_booking,
} = require("../controllers/booking_details");
const {
  addpayment_details,
  view_payment,
} = require("../controllers/addpayment_details");
const {
  inventory_details,
  view_inventory,
  remove_inventory,
  view_inventory_Bydeveloper,
  view_inventory_Bylocation,
  update_inventory,
} = require("../controllers/addinventory");
...
const {
  ...
  view_deal_Byproject,
  update_dealbyprojectandunitforownerdetails,
  getUnitDetails,
  updateMany,
  dealupdatemany,
  getGroupedDatadeal,
...
const {
  ...
  update_activity,
  view_activitybyid,
} = require("../controllers/addactivity");
...
const {
  ...
  rcvmessage,
} = require("../controllers/sendwhatsapp");
const {
  add_leadscore,
  view_leadscore,
  delete_leadscore,
  updateleadscore,
} = require("../controllers/leadscore");
...
router.put(
  "/updateleaddocumentsingle/:_id",
  upload.any("document_pic"),
  update_leadsingledocument
);
router.put(
  "/updateleadforbulkupload",
  upload.any("document_pic"),
  update_leadforbulkupload
);
...
router.get(
  "/viewprojectforinventories/:project_name/:unit_no/:block",
  view_projectforinventories
);
...
router.delete(
  "/deleteprojectforinventories/:project_name/:unit_no/:block",
  delete_projectforinventories
);
...
router.put(
  "/updatedealbysingle/:_id",
  upload.any("pic", "preview"),
  update_dealbysingle
);
...
router.put(
  "/updatedealowner/:project/:block/:unit_number",
  update_dealbyprojectandunitforownerdetails
);
...
router.get("/view_meeting_task", view_meeting_task);
